...
class ReleveVEApp:
    ...
    def charger_image_vehicule(self):
        ...
        if url:
            try:
                # Ajout d'un faux User-Agent de navigateur très complet pour contourner les protections anti-bots
                headers = {
                ...
                # Gestion propre de la transparence PNG
                if im.mode != 'RGBA':
                    im = im.convert('RGBA')
    ...
    def ouvrir_parametres(self):
        ...
        def chercher_web():
            # Ouverture directe de la galerie Auto & Moto de Seeklogo
    ...
    def afficher_a_propos(self, startup=False):
        ...
        explication = (
            ...
            "Nouveautés Récentes :\n"
            "  • [v8.6] Contournement des sécurités web pour l'affichage des logos.\n"
            "  • [v8.5] Refonte complète de l'interface en modules verticaux.\n"
            "  • [v8.4] Intégration des logos officiels constructeurs.\n"
            "  • [v8.2] Base de données multi-véhicules intégrée avec calcul ultra-précis.\n\n"
    ...
    def creer_interface(self):
        ...
        # --- 1A. Véhicule et Paramètres (Gauche) ---
        ...
        # --- 1C. Consommation du mois (Droite) ---
        ...
        # --- 2A. Formulaire de saisie (Gauche) ---
    ...
    def actualiser_liste_mois(self):
        ...
        if liste_mois:
            if not self.combo_mois_charge.get() in liste_mois:
                self.combo_mois_charge.set(liste_mois[-1])
            if not self.combo_mois_conso.get() in liste_mois:
                self.combo_mois_conso.set(liste_mois[-1])
        else:
    ...
    def _dessiner_graphique_conso(self):
        ...
        for i in range(len(coords)):
            x, y = coords[i]
            if i > 0:
                self.canvas_conso.create_line(coords[i-1][0], coords[i-1][1], x, y, fill=ACCENT_HOVER, width=2)

...
APP_DIR = os.path.expanduser("~/.local/share/ReleveVE")
DB_FILE = os.path.join(APP_DIR, ".releve_ve_data.db")
CONFIG_FILE = os.path.join(APP_DIR, ".releve_ve_config.json")
LOGOS_DIR = os.path.join(APP_DIR, "logos")

# --- Thème Light "Pure Flat" Adouci ---
BG_COLOR = "#e6ece8"       
...
class ReleveVEApp:
    def __init__(self, root):
        ...
        # Activation du clic droit sur les champs
        ...
        # Affichage du "A propos" uniquement au premier démarrage
        if self.premier_demarrage:
            self.root.after(100, lambda: self.afficher_a_propos(startup=True))
            self.premier_demarrage = False
            self.sauvegarder_config(self.prix_kwh_defaut)
    ...
    def sauvegarde_automatique(self):
        if os.path.exists(DB_FILE):
            backup_dir = os.path.join(APP_DIR, "Sauvegardes_VE")
        ...
        if os.path.exists(CONFIG_FILE):
            try:
                with open(CONFIG_FILE, "r", encoding="utf-8") as f:
                    ...
                    self.premier_demarrage = config.get("premier_demarrage", True)
            except:
                pass
        ...
        try:
            ...
            with open(CONFIG_FILE, "w", encoding="utf-8") as f:
                json.dump({
                    ...
                    "premier_demarrage": self.premier_demarrage
                }, f, indent=4)
        except:
            pass
    ...
    def initialiser_bdd(self):
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS releves (
            id INTEGER PRIMARY KEY AUTOINCREMENT, date TEXT, charge REAL, prix_kwh REAL, km REAL)''')
        
        cursor.execute("PRAGMA table_info(releves)")
        cols_releves = [col[1] for col in cursor.fetchall()]
        if "debut_charge" not in cols_releves: cursor.execute("ALTER TABLE releves ADD COLUMN debut_charge REAL DEFAULT 0")
        if "fin_charge" not in cols_releves: cursor.execute("ALTER TABLE releves ADD COLUMN fin_charge REAL DEFAULT 0")

        cursor.execute('''CREATE TABLE IF NOT EXISTS modeles_ve (
            id INTEGER PRIMARY KEY AUTOINCREMENT, marque TEXT, modele TEXT, capacite_utile REAL, image_url TEXT)''')
        
        cursor.execute("PRAGMA table_info(modeles_ve)")
        if "image_url" not in [col[1] for col in cursor.fetchall()]:
    ...
    def gerer_logo_local(self, url_ou_chemin, nom_vehicule):
        if not url_ou_chemin:
            return ""
            
        if os.path.exists(url_ou_chemin):
            return url_ou_chemin
            
        if url_ou_chemin.startswith("http"):
            try:
                nom_propre = "".join(c for c in nom_vehicule if c.isalnum() or c in (' ', '-')).strip().replace(' ', '_').lower()
                chemin_local = os.path.join(LOGOS_DIR, f"logo_{nom_propre}.png")
                
                headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/114.0.0.0 Safari/537.36'}
                req = urllib.request.Request(url_ou_chemin, headers=headers)
                ...
                im.save(chemin_local, "PNG")
                return chemin_local
            except Exception as e:
                print(f"Échec du téléchargement du logo : {e}")
                return url_ou_chemin
        
        return url_ou_chemin

    def charger_image_vehicule(self):
        chemin = self.vehicule_image_url
        if chemin:
            try:
                if os.path.exists(chemin):
                    im = Image.open(chemin)
                elif chemin.startswith("http"):
                    headers = {'User-Agent': 'Mozilla/5.0'}
                    req = urllib.request.Request(chemin, headers=headers)
                    raw_data = urllib.request.urlopen(req, timeout=5).read()
                    im = Image.open(io.BytesIO(raw_data))
                else:
                ...
                self.lbl_image.config(image='', text="🏷️\n(Logo introuvable)", font=("Arial", 12))
        else:
    ...
    def ouvrir_parametres(self):
        ...
        tk.Label(frame_manuel, text="Lien ou chemin du Logo :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=1, column=0, sticky="e", pady=5)
        ...
        def chercher_web():
            url_recherche = "https://seeklogo.com/free-vector-logos/auto-and-moto?type=brand"
            webbrowser.open(url_recherche)
            
        tk.Button(frame_manuel, text="🌐 Ouvrir la galerie Seeklogo", bg=BG_COLOR, fg=TEXT_MUTED, relief=tk.FLAT, bd=0, font=("Arial", 8), command=chercher_web).grid(row=2, column=1, sticky="w", pady=(0, 10))
        ...
        def sauvegarder():
            try:
                ...
                chemin_logo_final = self.gerer_logo_local(entree_url, nom_vehicule)
                
                if idx >= 0:
                    cursor.execute("UPDATE modeles_ve SET image_url=? WHERE id=?", (chemin_logo_final, vehicules[idx][4] if len(vehicules[idx])>4 else idx+1))
                    conn.commit()
                conn.close()

                self.sauvegarder_config(self.prix_kwh_defaut, capacite=nouvelle_cap, nom=nom_vehicule, url_image=chemin_logo_final)
            except ValueError:
    ...
    def afficher_a_propos(self, startup=False):
        ...
        explication = (
            ...
            "Nouveautés Récentes :\n"
            "  • [v8.9] Bouton d'export PDF rapide et fenêtre d'accueil masquée.\n"
            "  • [v8.8] Téléchargement et base de données locale pour les logos hors-ligne.\n"
            "  • [v8.7] Intégration de la galerie SeekLogo.\n"
            "  • [v8.5] Refonte complète de l'interface en modules verticaux.\n\n"
    ...
    def creer_interface(self):
        ...
        # ==========================================
        # SECTION 1 : LES 3 FENÊTRES DU HAUT
        # ==========================================
        frame_top = tk.Frame(self.root, bg=BG_COLOR)
        frame_top.pack(fill=tk.X, padx=10, pady=(10, 5))

        frame_vehicule = tk.Frame(frame_top, bg=SURFACE_COLOR, bd=0)
        frame_vehicule.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 5))
        
        btn_changer = tk.Button(frame_vehicule, text="⚙️ Paramètres", bg=ACCENT_COLOR, fg=TEXT_COLOR, relief=tk.FLAT, font=("Arial", 9, "bold"), command=self.ouvrir_parametres, padx=10, pady=5)
        btn_changer.pack(anchor="ne", padx=10, pady=10)

        self.lbl_image = tk.Label(frame_vehicule, bg=SURFACE_COLOR, text="🏷️", font=("Arial", 40))
        self.lbl_image.pack(pady=(0, 5))
        
        self.lbl_nom_vehicule = tk.Label(frame_vehicule, text=self.vehicule_nom, bg=SURFACE_COLOR, fg=TEXT_COLOR, font=("Arial", 14, "bold"))
        self.lbl_nom_vehicule.pack()
        
        self.lbl_cap_vehicule = tk.Label(frame_vehicule, text=f"Capacité utile : {self.capacite_batterie} kWh", bg=SURFACE_COLOR, fg=TEXT_MUTED, font=("Arial", 10))
        self.lbl_cap_vehicule.pack(pady=(0, 15))

        frame_charge = tk.Frame(frame_top, bg=SURFACE_COLOR)
        frame_charge.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=5)
        
        frame_titre_g = tk.Frame(frame_charge, bg=SURFACE_COLOR)
        frame_titre_g.pack(fill=tk.X, pady=(10, 5))
        tk.Label(frame_titre_g, text="Charges :", bg=SURFACE_COLOR, fg=TEXT_COLOR, font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=(10, 5))
        
        self.combo_mois_charge = ttk.Combobox(frame_titre_g, state="readonly", width=10)
        self.combo_mois_charge.pack(side=tk.LEFT)
        self.combo_mois_charge.bind("<<ComboboxSelected>>", lambda e: self.dessiner_graphiques())
        
        self.canvas_charge = tk.Canvas(frame_charge, bg=SURFACE_COLOR, height=150, bd=0, highlightthickness=0)
        self.canvas_charge.pack(fill=tk.BOTH, expand=True)
        
        self.lbl_total_charge = tk.Label(frame_charge, text="Total Charge : -- kWh | Prix : -- €", bg=SURFACE_COLOR, fg=TEXT_COLOR, font=("Arial", 10, "bold"))
        self.lbl_total_charge.pack(pady=(5, 10))

        frame_conso = tk.Frame(frame_top, bg=SURFACE_COLOR)
        frame_conso.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(5, 0))
        
        frame_titre_d = tk.Frame(frame_conso, bg=SURFACE_COLOR)
        frame_titre_d.pack(fill=tk.X, pady=(10, 5))
        tk.Label(frame_titre_d, text="Consommation :", bg=SURFACE_COLOR, fg=TEXT_COLOR, font=("Arial", 10, "bold")).pack(side=tk.LEFT, padx=(10, 5))
        
        self.combo_mois_conso = ttk.Combobox(frame_titre_d, state="readonly", width=10)
        self.combo_mois_conso.pack(side=tk.LEFT)
        self.combo_mois_conso.bind("<<ComboboxSelected>>", lambda e: self.dessiner_graphiques())
        
        self.canvas_conso = tk.Canvas(frame_conso, bg=SURFACE_COLOR, height=150, bd=0, highlightthickness=0)
        self.canvas_conso.pack(fill=tk.BOTH, expand=True)
        
        self.lbl_moyenne_conso = tk.Label(frame_conso, text="Moyenne Conso : -- kWh/100km", bg=SURFACE_COLOR, fg=TEXT_COLOR, font=("Arial", 10, "bold"))
        self.lbl_moyenne_conso.pack(pady=(5, 10))

        self.canvas_charge.bind("<Configure>", lambda e: self.dessiner_graphiques())
        self.canvas_conso.bind("<Configure>", lambda e: self.dessiner_graphiques())

        # ==========================================
        # SECTION 2 : LES 2 FENÊTRES DU MILIEU
        # ==========================================
        frame_middle = tk.Frame(self.root, bg=BG_COLOR)
        frame_middle.pack(fill=tk.BOTH, padx=10, pady=5, expand=False)

        frame_saisie = tk.Frame(frame_middle, bg=SURFACE_COLOR, bd=0)
        frame_saisie.pack(side=tk.LEFT, fill=tk.Y, expand=False, padx=(0, 5))
        
        tk.Label(frame_saisie, text="Nouvel Enregistrement", bg=SURFACE_COLOR, fg=TEXT_COLOR, font=("Arial", 11, "bold")).grid(row=0, column=0, columnspan=2, pady=10)

        entry_kwargs = {"bg": BG_COLOR, "fg": TEXT_COLOR, "insertbackground": TEXT_COLOR, "relief": tk.FLAT, "bd": 0, "highlightthickness": 0}

        tk.Label(frame_saisie, text="Date :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=1, column=0, padx=(15, 5), pady=8, sticky="e")
        self.ent_date = tk.Entry(frame_saisie, width=15, **entry_kwargs)
        self.ent_date.grid(row=1, column=1, padx=(0, 15), pady=8, sticky="w")
        self.ent_date.insert(0, datetime.date.today().strftime("%d/%m/%Y"))

        tk.Label(frame_saisie, text="Début (%) :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=2, column=0, padx=(15, 5), pady=8, sticky="e")
        self.ent_debut = tk.Entry(frame_saisie, width=15, **entry_kwargs)
        self.ent_debut.grid(row=2, column=1, padx=(0, 15), pady=8, sticky="w")

        tk.Label(frame_saisie, text="Fin (%) :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=3, column=0, padx=(15, 5), pady=8, sticky="e")
        self.ent_fin = tk.Entry(frame_saisie, width=15, **entry_kwargs)
        self.ent_fin.grid(row=3, column=1, padx=(0, 15), pady=8, sticky="w")

        tk.Label(frame_saisie, text="Charge (kWh) :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=4, column=0, padx=(15, 5), pady=8, sticky="e")
        self.ent_charge = tk.Entry(frame_saisie, width=15, **entry_kwargs)
        self.ent_charge.grid(row=4, column=1, padx=(0, 15), pady=8, sticky="w")

        tk.Label(frame_saisie, text="Km total :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=5, column=0, padx=(15, 5), pady=8, sticky="e")
        self.ent_km = tk.Entry(frame_saisie, width=15, **entry_kwargs)
        self.ent_km.grid(row=5, column=1, padx=(0, 15), pady=8, sticky="w")

        tk.Label(frame_saisie, text="Prix kWh (€) :", bg=SURFACE_COLOR, fg=TEXT_COLOR).grid(row=6, column=0, padx=(15, 5), pady=8, sticky="e")
        self.ent_prix_kwh = tk.Entry(frame_saisie, width=15, **entry_kwargs)
        self.ent_prix_kwh.grid(row=6, column=1, padx=(0, 15), pady=8, sticky="w")
        if self.prix_kwh_defaut:
            self.ent_prix_kwh.insert(0, self.prix_kwh_defaut)

        frame_btns_saisie = tk.Frame(frame_saisie, bg=SURFACE_COLOR)
        frame_btns_saisie.grid(row=7, column=0, columnspan=2, pady=15)
        
        self.btn_ajouter = tk.Button(frame_btns_saisie, text="Ajouter", bg=ACCENT_COLOR, fg=TEXT_COLOR, relief=tk.FLAT, bd=0, font=("Arial", 10, "bold"), command=self.valider_saisie, padx=15)
        self.btn_ajouter.pack(side=tk.LEFT, padx=5)

        self.btn_annuler = tk.Button(frame_btns_saisie, text="X", bg=DANGER_COLOR, fg="white", relief=tk.FLAT, bd=0, font=("Arial", 10, "bold"), command=self.reinitialiser_formulaire, padx=5)
        self.btn_annuler.pack(side=tk.LEFT, padx=5)
        self.btn_annuler.pack_forget()

        frame_vide = tk.Frame(frame_middle, bg=SURFACE_COLOR)
        frame_vide.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(5, 0))
        tk.Label(frame_vide, text="[ Espace réservé aux futures fonctionnalités ]", fg=TEXT_MUTED, bg=SURFACE_COLOR, font=("Arial", 10, "italic")).pack(expand=True)

        # ==========================================
        # SECTION 3 : TABLEAU ET FILTRE (Bas)
        # ==========================================
        frame_bottom = tk.Frame(self.root, bg=BG_COLOR)
        frame_bottom.pack(fill=tk.BOTH, expand=True, padx=10, pady=(5, 10))

        frame_filtre = tk.Frame(frame_bottom, bg=BG_COLOR)
        frame_filtre.pack(fill=tk.X, pady=(0, 5))
        ...
        # --- NOUVEAUTÉ : Bouton Exporter PDF ---
        btn_export_pdf = tk.Button(frame_filtre, text="📄 Exporter PDF", bg=ACCENT_COLOR, fg=TEXT_COLOR, relief=tk.FLAT, bd=0, font=("Arial", 9, "bold"), command=self.exporter_pdf)
        btn_export_pdf.pack(side=tk.LEFT, padx=5)

        colonnes = ("date", "debut_charge", "fin_charge", "charge", "prix_kwh", "km", "conso_moyenne", "cout_total")
        self.tree = ttk.Treeview(frame_bottom, columns=colonnes, show="headings", height=8)
    ...
    def sur_selection_ligne(self, event):
        ...
        if selection: self.preparer_modification()
    ...
    def actualiser_liste_mois(self):
        if not hasattr(self, "combo_mois"): return
        mois_presents = set()
        for d in self.donnees:
            try: mois_presents.add(d["date"][3:])
        ...
        if self.filtre_mois_courant not in liste_filtre: self.filtre_mois_courant = "Tous les mois"
        self.combo_mois.set(self.filtre_mois_courant)
        
        self.combo_mois_charge["values"] = liste_mois
        self.combo_mois_conso["values"] = liste_mois
        
        if liste_mois:
            if not self.combo_mois_charge.get() in liste_mois: self.combo_mois_charge.set(liste_mois[-1])
            if not self.combo_mois_conso.get() in liste_mois: self.combo_mois_conso.set(liste_mois[-1])
        else:
            self.combo_mois_charge.set("")
            self.combo_mois_conso.set("")
    ...
    def charger_donnees(self):
        ...
        for ligne in lignes:
            self.donnees.append({
                "id": ligne[0], "date": ligne[1], "debut_charge": ligne[2],
                "fin_charge": ligne[3], "charge": ligne[4], "prix_kwh": ligne[5], "km": ligne[6]
            })
        self.recalculer_donnees()
        self.actualiser_liste_mois()
    ...
    def valider_saisie(self):
        ...
        if date_val and debut and fin and charge and prix_kwh and km:
            try:
                debut_f, fin_f, charge_f = float(debut), float(fin), float(charge)
                prix_kwh_f, km_f = float(prix_kwh), float(km)
                ...
                if self.id_edition is not None:
                    cursor.execute('''UPDATE releves SET date=?, debut_charge=?, fin_charge=?, charge=?, prix_kwh=?, km=? WHERE id=?''', 
                                   (date_val, debut_f, fin_f, charge_f, prix_kwh_f, km_f, self.id_edition))
                else:
                    cursor.execute('''INSERT INTO releves (date, debut_charge, fin_charge, charge, prix_kwh, km) VALUES (?, ?, ?, ?, ?, ?)''', 
    ...
    def preparer_modification(self):
        ...
        self.btn_ajouter.config(text="Valider", bg=WARNING_COLOR, fg=SURFACE_COLOR)
        self.btn_annuler.pack(side=tk.LEFT, padx=5)
    ...
    def recalculer_donnees(self):
        for i in range(len(self.donnees)):
            charge, prix_kwh = float(self.donnees[i]["charge"]), float(self.donnees[i]["prix_kwh"])
            self.donnees[i]["cout_total"] = round(charge * prix_kwh, 2)
            
            if i == 0:
                self.donnees[i]["conso_100km"] = "N/A"
            else:
                km_actuel, km_prec = float(self.donnees[i]["km"]), float(self.donnees[i-1]["km"])
        ...
        for d in self.donnees:
            if self.filtre_mois_courant == "Tous les mois" or d["date"].endswith(self.filtre_mois_courant):
                self.tree.insert("", tk.END, iid=str(d["id"]), values=(
                    d["date"], f"{d.get('debut_charge', 0):g}%", f"{d.get('fin_charge', 0):g}%", 
                    d["charge"], d["prix_kwh"], d["km"], d.get("conso_100km", ""), d.get("cout_total", "")
                ))
        self.dessiner_graphiques()

    def dessiner_graphiques(self):
        self._dessiner_graphique_charge()
        self._dessiner_graphique_conso()

    def _dessiner_graphique_charge(self):
        self.canvas_charge.delete("all")
        mois_choisi = self.combo_mois_charge.get()
        
        if not mois_choisi:
            self.canvas_charge.create_text(self.canvas_charge.winfo_width()//2, 80, text="Sélectionnez un mois pour voir les jours", fill=TEXT_MUTED, font=("Arial", 10, "italic"))
            self.lbl_total_charge.config(text="Total Charge : -- kWh  |  Total Prix : -- €")
            return

        data_jours = defaultdict(float)
        total_charge, total_prix = 0.0, 0.0
        
        for d in self.donnees:
            if d["date"].endswith(mois_choisi):
                jour = d["date"][:2]
                charge, prix = float(d.get("charge", 0)), float(d.get("cout_total", 0))
                data_jours[jour] += charge
                total_charge += charge
                total_prix += prix
                
        self.lbl_total_charge.config(text=f"Total : {total_charge:.2f} kWh  |  Prix : {total_prix:.2f} €")
        
        if not data_jours:
            self.canvas_charge.create_text(self.canvas_charge.winfo_width()//2, 80, text="Aucune donnée pour ce mois", fill=TEXT_MUTED, font=("Arial", 10, "italic"))
            return

        largeur, hauteur = self.canvas_charge.winfo_width(), self.canvas_charge.winfo_height()
        if largeur <= 1: largeur = 350
        if hauteur <= 1: hauteur = 150

        marge_x, marge_y = 30, 25
        zone_l, zone_h = largeur - 2 * marge_x, hauteur - 2 * marge_y

        self.canvas_charge.create_line(marge_x, hauteur - marge_y, largeur - marge_x + 10, hauteur - marge_y, fill=TEXT_MUTED, width=2)
        ...
        largeur_barre = min(pas_x * 0.6, 30)
        
        for i, jour in enumerate(jours):
            val = data_jours[jour]
            x_centre = marge_x + i * pas_x + pas_x / 2
            h_barre = (val / plafond) * zone_h
            x0, y0 = x_centre - largeur_barre / 2, hauteur - marge_y - h_barre
            x1, y1 = x_centre + largeur_barre / 2, hauteur - marge_y
            
            self.canvas_charge.create_rectangle(x0, y0, x1, y1, fill=ACCENT_COLOR, outline=ACCENT_COLOR)
            self.canvas_charge.create_text(x_centre, y0 - 10, text=f"{val:g}", font=("Arial", 8, "bold"), fill=TEXT_COLOR)
            self.canvas_charge.create_text(x_centre, hauteur - marge_y + 12, text=jour, font=("Arial", 8), fill=TEXT_MUTED)

    def _dessiner_graphique_conso(self):
        self.canvas_conso.delete("all")
        mois_choisi = self.combo_mois_conso.get()
        
        if not mois_choisi:
            self.canvas_conso.create_text(self.canvas_conso.winfo_width()//2, 80, text="Sélectionnez un mois", fill=TEXT_MUTED, font=("Arial", 10, "italic"))
            self.lbl_moyenne_conso.config(text="Moyenne Conso : -- kWh/100km")
            return
            
        points_conso, somme_conso = [], 0.0
        
        for d in self.donnees:
            if d["date"].endswith(mois_choisi) and d.get("conso_100km") not in ("N/A", ""):
                jour = d["date"][:2]
                val = float(d["conso_100km"])
                points_conso.append((jour, val))
                somme_conso += val
                
        if not points_conso:
            self.canvas_conso.create_text(self.canvas_conso.winfo_width()//2, 80, text="Aucune donnée de conso pour ce mois", fill=TEXT_MUTED, font=("Arial", 10, "italic"))
            self.lbl_moyenne_conso.config(text="Moyenne Conso : -- kWh/100km")
            return
            
        moyenne = somme_conso / len(points_conso)
        self.lbl_moyenne_conso.config(text=f"Moyenne du mois : {moyenne:.2f} kWh/100km")

        largeur, hauteur = self.canvas_conso.winfo_width(), self.canvas_conso.winfo_height()
        if largeur <= 1: largeur = 350
        if hauteur <= 1: hauteur = 150
        ...
        self.canvas_conso.create_line(marge_x, hauteur - marge_y, largeur - marge_x + 10, hauteur - marge_y, arrow=tk.LAST, fill=TEXT_MUTED, width=2)
        self.canvas_conso.create_line(marge_x, hauteur - marge_y, marge_x, marge_y - 10, arrow=tk.LAST, fill=TEXT_MUTED, width=2)

        valeurs = [p[1] for p in points_conso]
        val_max = max(valeurs) if max(valeurs) > 0 else 1
        plafond = val_max * 1.2 
        pas_x = zone_dessin_l / (len(points_conso) - 1) if len(points_conso) > 1 else zone_dessin_l / 2

        coords = []
        for i, (jour, val) in enumerate(points_conso):
            x = marge_x + i * pas_x if len(points_conso) > 1 else marge_x + pas_x
            y = (hauteur - marge_y) - (val / plafond) * zone_dessin_h
            coords.append((x, y))
            self.canvas_conso.create_text(x, y - 12, text=f"{val}", font=("Arial", 8, "bold"), fill=TEXT_COLOR)
            self.canvas_conso.create_text(x, hauteur - marge_y + 12, text=jour, font=("Arial", 8), fill=TEXT_MUTED)

        for i in range(len(coords)):
            x, y = coords[i]
            if i > 0: self.canvas_conso.create_line(coords[i-1][0], coords[i-1][1], x, y, fill=ACCENT_HOVER, width=2)
            self.canvas_conso.create_oval(x - 4, y - 4, x + 4, y + 4, fill=ACCENT_COLOR, outline=SURFACE_COLOR, width=1)

    def _get_documents_dir(self):
        docs_dir = os.path.expanduser("~/Documents")
        return docs_dir if os.path.exists(docs_dir) else os.path.expanduser("~")

    def generer_modele_vierge(self):
        filepath = filedialog.asksaveasfilename(defaultextension=".pdf", initialdir=self._get_documents_dir(),
                                                initialfile="Modele_Releve.pdf", title="Enregistrer le modèle vierge dans Documents")
        if filepath: self._creer_pdf(filepath, vierge=True)
    ...
    def exporter_pdf(self):
        ...
        donnees_a_exporter, total_kwh, total_euros = [], 0.0, 0.0
        ...
        filepath = filedialog.asksaveasfilename(defaultextension=".pdf", initialdir=self._get_documents_dir(),
                                                initialfile=nom_fichier, title="Exporter en PDF dans Documents")
        if filepath: self._creer_pdf(filepath, vierge=False, donnees=donnees_a_exporter, totaux=(total_kwh, total_euros), periode=self.filtre_mois_courant)
    ...
    def _creer_pdf(self, filepath, vierge, donnees=None, totaux=(0,0), periode=""):
        ...
        pdf.set_text_color(45, 56, 48)
        pdf.cell(200, 15, "Relevé Manuel - Charge VE" if vierge else f"Historique des Relevés - {periode}", 0, 1, 'C')
        pdf.ln(3)

        pdf.set_fill_color(156, 191, 163)
        pdf.set_text_color(0, 0, 0)
        pdf.set_font("Arial", 'B', 9)
        
        if vierge:
            for w, txt in [(30, "Date"), (25, "Debut (%)"), (25, "Fin (%)"), (40, "Charge (kWh)"), (70, "Kilometrage")]:
                pdf.cell(w, 10, txt, 1, 0 if txt != "Kilometrage" else 1, 'C', fill=True)
        else:
            for w, txt in [(25, "Date"), (20, "Debut"), (20, "Fin"), (25, "Charge"), (20, "Prix kWh"), (40, "Kilometrage"), (40, "Cout Total")]:
                pdf.cell(w, 10, txt, 1, 0 if txt != "Cout Total" else 1, 'C', fill=True)
        ...
        if vierge:
            for _ in range(20):
                ...
                for w in [30, 25, 25, 40]: pdf.cell(w, 10, "", 1, 0, 'C', fill=True)
